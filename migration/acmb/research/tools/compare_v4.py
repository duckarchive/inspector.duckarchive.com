...
TYPO = json.load(open(f"{S}/typos.json")).get(A, {}) if __import__("os").path.exists(f"{S}/typos.json") else {}
...
def yrs(s):
    ...
    return o
R = []; P = R.append
P(f"# {A} vs catalog — {len(PR)} справи in {len({k.split('-')[0] for k in PR})} fonds / {len({tuple(k.split('-')[:2]) for k in PR})} описи")
...
# partial loads: catalog cites справи beyond what the DB holds for that опис
partial = []
for (f, op), mx in maxspr.items():
    db_codes = [int(c) for (ff, oo, c) in DFL if ff == f and oo == op and c.isdigit()]
    db_max = max(db_codes, default=0); n_ref = sum(1 for k in PR if k.startswith(f"{f}-{op}-")); n_miss = sum(1 for k in miss_file if k.startswith(f"{f}-{op}-"))
    if n_miss and (mx > db_max or n_miss > 20): partial.append((f"{f}-{op}", len(db_codes), db_max, n_ref, mx, n_miss))
partial.sort(key=lambda x: -x[5])
P(f"- описи that look partially loaded (catalog cites справи the DB lacks): {len(partial)} → " + "; ".join(f"{k}: DB {n} files (max {dbm}), catalog {nr} refs (max {cm}), missing {nm}" for k, n, dbm, nr, cm, nm in partial[:8]))
# ---- action lists ----
